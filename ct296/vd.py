...
CSV_PATH      = "energydata_complete.csv"  # đường dẫn file
TEST_SIZE     = 0.3       # 30 % làm test
N_NEIGHBORS   = 5         # k của K-NN
IMPUTE_STRAT  = "median"  # chọn cách lấp (impute) giá trị NaN – ở đây lấy median (trung vị) của mỗi cột.
DELTA_ACCUR   = 50        # tự định nghĩa độ đo “Accuracy ±δ”: dự đoán được coi là đúng nếu sai số nằm trong ±50 Wh.
...
X_train, X_test, y_train, y_test = train_test_split(
    X, y, test_size=TEST_SIZE, random_state=42 )    #Chia ngẫu nhiên (seed = 42)
...
# Accuracy ±δ
metrics[f'Acc ±{DELTA_ACCUR}Wh'] = (
    (np.abs(y_test - y_pred) <= DELTA_ACCUR).mean()
)
...
# ========= 8. Ví dụ dự đoán mẫu mới =========
print("\n>>> Dự đoán mẫu mới (có thể thiếu dữ liệu)")
new_sample = X_test.iloc[0].copy()
new_sample['RH_1'] = np.nan      # cố tình thiếu dữ liệu (có thể bỏ qua)
y_hat = pipe.predict(pd.DataFrame([new_sample]))
print("Tiêu thụ năng lượng dự đoán:", y_hat[0], "Wh")
